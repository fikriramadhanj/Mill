;(function($, window, numeral) {
  function changeTempoDuration () {
    var elDateFaktur = $('.fb-tanggal-faktur');
    var elDateJatuhTempo = $('.fb-tanggal-jatuh-tempo');
    ...
    $('#fb-tempo-bayar').val(duration);
  }

  function editRowBeforeAppend(clonedRow, index) {
    var changes = [
      {
        className: '.fb-hapus-barang',
        attr: 'data-index',
        value: index,
        type: 'button'
      },
      {
        ...
        attr: 'name',
        value: `detilBeli[${index}][subTotal]`,
        type: 'input'
      },
      {
        className: '.select-barang',
        attr: 'name',
        value: `detilBeli[${index}][barangId]`,
        type: 'select'
      },
      {
        className: '.barang-qty-input',
        attr: 'name',
        value: `detilBeli[${index}][qty]`,
        type: 'inputNumber'
      },
      {
  ...
  function calculateTotal() {
    var total = {
      qty: {
        el: $('#faktur-beli-total-qty'),
        formatter: function (a) { return a; },
        value: 0
      },
      subTotal: {
        el: $('#faktur-beli-total-subTotal'),
  ...
  $('.fb-tambah-barang').on('click', function () {
    var elTableBody = $('#fb-barang-list');
  ...
  $('.fb-hapus-barang').on('click', function () {
    var el = $(this);
  ...
  // Tanggal Faktur
  $('.fb-tanggal-faktur').on('changeDate', function (e) {
    changeTempoDuration();
  });

  $('.fb-tanggal-jatuh-tempo').on('changeDate', function () {
    changeTempoDuration();
  });
